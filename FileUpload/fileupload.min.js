/*
 * FileUpload GeneXus User Control 0.0.0.141366
 *
...
0,f),function(f){if(f.target.error)console.log(f.target.error);else{f=f.target.result;var g=new DataView(f),k=2,l=g.byteLength-4,n=k,p,q;if(65496===g.getUint16(0)){for(;k<l;)if(p=g.getUint16(k),65504<=p&&65519>=p||65534===p){q=g.getUint16(k+2)+2;if(k+q>g.byteLength){console.log("Invalid meta data: Invalid segment size.");break}if(n=b.metaDataParsers.jpeg[p])for(p=0;p<n.length;p+=1)n[p].call(d,g,k,q,e,c);n=k+=q}else break;!c.disableImageHead&&6<n&&(e.imageHead=f.slice?f.slice(0,n):(new Uint8Array(f)).subarray(0,
...
return f?f.abort():(h&&h.rejectWith(k.context,e),l())},this._enhancePromise(g)):l()},_onAdd:function(a,c){var d=this,f=!0,e=b.extend({},this.options,c),h=c.files,g=h.length,k=e.limitMultiFileUploads,l=e.limitMultiFileUploadSize,n=e.limitMultiFileUploadSizeOverhead,p=0,q=this._getParamName(e),u,r,t=0;if(!g)return!1;l&&void 0===h[0].size&&(l=void 0);if((e.singleFileUploads||k||l)&&this._isXHRUpload(e))if(e.singleFileUploads||l||!k)if(!e.singleFileUploads&&l)for(r=[],u=[],e=0;e<g;e+=1){if(p+=h[e].size+
n,e+1===g||p+h[e+1].size+n>l||k&&e+1-t>=k)r.push(h.slice(t,e+1)),p=q.slice(t,e+1),p.length||(p=q),u.push(p),t=e+1,p=0}else u=q;else for(r=[],u=[],e=0;e<g;e+=k)r.push(h.slice(e,e+k)),p=q.slice(e,e+k),p.length||(p=q),u.push(p);else r=[h],u=[q];c.originalFiles=h;b.each(r||h,function(e,h){var g=b.extend({},c);g.files=r?h:[h];g.paramName=u[e];d._initResponseObject(g);d._initProgressObject(g);d._addConvenienceMethods(a,g);return f=d._trigger("add",b.Event("add",{delegatedEvent:a}),g)});return f},_replaceFileInput:function(a){var c=
...
do{if(a===g[0]){b=!0;break}a=a.parentNode}while(a);b?g.addClass("hover"):g.removeClass("hover");window.dropZoneTimeout=setTimeout(function(){window.dropZoneTimeout=null;g.removeClass("in hover")},100)})},_create:function(){this._super();var b=this.element,a=this,c=function(){var c=b.find(".files"),f=c.children().length,f=0<a.options.maxNumberOfFiles&&f>=a.options.maxNumberOfFiles,e=c.children(".template-upload").not(".processing").length,h=c.children(".template-upload.processing").length,m=c.children(".template-download").not(".processing").length,
c=c.children(".template-download").not(".processing").find(".toggle:checked").length,k=b.find(".fileupload-buttonbar");k.find(".start").prop("disabled",0===e);k.find(".cancel").prop("disabled",0===e&&0===h);k.find(".delete").prop("disabled",0===c);k.find(".toggle").prop("disabled",0===m);a.options.autoDisableAdd&&(f?(a._disableFileInputButton(),a.options.dropZone.hide()):(a._enableFileInputButton(),a.options.dropZone.show()))};b.bind("fileuploadfailed fileuploadadded fileuploaddestroyed fileuploadcompleted",
c);b.on("click",".toggle",c);b.bind("fileuploadcompleted",function(a,c){(b.data("blueimp-fileupload")||b.data("fileupload"))._renderPreviews(c)});c();this._defineDropZone()},_destroy:function(){b(document).unbind("dragover").undelegate("dragover")}})});gx.uc.FileUpload=function(b){var g=[],a,c=gx.lang.gxBoolean,d=function(a,b){b=b||"";switch(a){case "image":return/(\.|\/)(gif|jpe?g|png)$/i;case "audio":return/(\.|\/)(mp3|wav|ogg|3gp|aac|wma)$/i;case "video":return/(\.|\/)(avi|flv|wmv|mov|mp4)$/i;case "custom":if(b)return new RegExp("(\\.|\\/)("+b.replace(/,/g,"|")+")$","i")}};this.getFiles=function(){return b.map(g,function(a){if(!a.destroyed)return{FullName:a.name,Name:a.name.replace(new RegExp("."+a.extension+"$","i"),""),Extension:a.extension,
Size:a.size,File:a.path}})};this.show=function(){gx.uc.FileUpload.initTemplates();if(!this.IsPostBack){var f=b(this.getContainerControl());f.attr("id");var e=this.AcceptedFileTypes,h=this.CustomFileTypes,m="",k="",l=d(e,h),n=this.Class||"",p=0,q=0;"image"===e||"audio"===e||"video"===e?m=' accept\x3d"'+e+'/*"':h&&(m=' accept\x3d".'+h.split(",").join(",.")+'"');k=1===this.MaxNumberOfFiles?"":" multiple";a=b(['\x3cdiv class\x3d"gx-fileupload ',n,'"\x3e\n\x3cdiv class\x3d"row fileupload-buttonbar"\x3e\n\x3cdiv class\x3d"col-lg-7"\x3e\n\x3cspan class\x3d"btn btn-success fileinput-button" title\x3d"',
this.TooltipText,'" \x3e\n\x3ci class\x3d"glyphicon glyphicon-plus"\x3e\x3c/i\x3e\n\x3cspan\x3e',gx.getMessage("GXM_fileupload_addfiles"),'\x3c/span\x3e\n\x3cinput type\x3d"file" name\x3d"files[]"',k,m,'\x3e\n\x3c/span\x3e\n\x3cbutton type\x3d"submit" class\x3d"btn btn-primary start"\x3e\n\x3ci class\x3d"glyphicon glyphicon-upload"\x3e\x3c/i\x3e\n\x3cspan\x3e',gx.getMessage("GXM_fileupload_start"),'\x3c/span\x3e\n\x3c/button\x3e\n\x3cbutton type\x3d"reset" class\x3d"btn btn-warning cancel"\x3e\n\x3ci class\x3d"glyphicon glyphicon-ban-circle"\x3e\x3c/i\x3e\n\x3cspan\x3e',
...
gx.getMessage("GXM_fileupload_drophere"),'\x3c/span\x3e\n\x3c/div\x3e\n\x3ctable role\x3d"presentation" class\x3d"table table-striped"\x3e\x3ctbody class\x3d"files"\x3e\x3c/tbody\x3e\x3c/table\x3e\n\x3c/div\x3e'].join("\n"));f.append(a);a.fileupload({url:gx.gen.isDotNet()?"gxobject.aspx":"../gxobject",autoUpload:c(this.AutoUpload)});a.bind("fileuploadadded",function(){p++}.closure(this));a.bind("fileuploaddestroyed",function(a,b){var c=b.context.data("index");g[c].destroyed=!0});a.bind("fileuploadcompleted",
function(){q++;q===p&&this.UploadComplete&&this.UploadComplete()}.closure(this));f={disableImageResize:/Android(?!.*Chrome)|Opera/.test(window.navigator.userAgent),maxFileSize:this.MaxFileSize,getFilesFromResponse:function(a){for(var b,c=0;c<a.result.files.length;c++)b=a.result.files[c],b._index=q,g.push(b);return a.result.files},messages:{maxNumberOfFiles:gx.getMessage("GXM_fileupload_maxNumberOfFiles"),acceptFileTypes:gx.getMessage("GXM_fileupload_acceptFileTypes"),maxFileSize:gx.getMessage("GXM_fileupload_maxFileSize")},
showCancelDownloadButton:this.EnableUploadedFileCanceling,autoDisableAdd:this.AutoDisableAddingFiles};0<this.MaxNumberOfFiles&&(f.maxNumberOfFiles=this.MaxNumberOfFiles);l&&(f.acceptFileTypes=l);a.fileupload("option",f)}};this.destroy=function(){a&&(a.fileupload("destroy"),b.cleanData(a))};this.Clear=function(){a.find("table tbody.files").empty();g=[]};this.StartUpload=function(){a.find(".start").click()}};
